

#include <Adafruit_SSD1306.h>

Adafruit_SSD1306 display(128,32,&Wire, -1);

// 'DinossauroBebadologoBWl', 103x64px
const unsigned char DinossauroBebadoLogo [] PROGMEM = {
  
};
// 'happy', 128x32px
const unsigned char happy [] PROGMEM = {
  
};



void setup()   
{                
  // initialize with the I2C addr 0x3C
  display.begin(SSD1306_SWITCHCAPVCC, 0x3C);  
  Serial.begin(57600);
  // Clear the buffer.
  display.clearDisplay();

  // Display bitmap
  display.drawBitmap(10, 0,  DinossauroBebadoLogo, 103, 64, WHITE);
 
  display.fillRect(0, 0, 10, 64, WHITE);
   display.fillRect(113, 0, 20, 64, WHITE);
   display.display();

  // Invert Display
 
}

void loop() {
 // display.clearDisplay();
 // display.fillCircle(64,16,10, WHITE);
  //display.fillCircle(64,16, 6,BLACK);
  //display.display();
  //delay(1000);
  //display.fillRect(50,-25, 40,40, BLACK);
  //display.display();
  //delay(800);
  display.clearDisplay();
  display.fillCircle(64,16,50, WHITE);
  display.fillCircle(64,16, 20,BLACK);
  display.fillCircle(67,14,5,WHITE);
  display.display();
 // display.fillCircle(64,20, 10, BLACK);
  //display.display();
 // delay(800);
 // display.clearDisplay();
  
  
 }
 
